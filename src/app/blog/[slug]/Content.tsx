      ...
      <section className="bg-gradient-to-b from-blue-50 via-white to-white">
        <div className="container-custom py-12 md:py-16">
          ...
          <div className="mt-6 max-w-3xl">
            <Badge className="bg-blue-100 text-blue-700 border-blue-200 mb-3">{post.category}</Badge>
            <h1 className="text-2xl md:text-4xl font-black text-blue-950 mb-4 leading-tight">{post.title}</h1>
        ...
        <div className="container-custom">
          <div className="grid lg:grid-cols-3 gap-8">
            <div className="lg:col-span-2">
              <div className="relative aspect-[16/9] rounded-2xl overflow-hidden mb-8 border border-blue-100/60">
              ...
              {post.tags.length > 0 && (
                <div className="flex flex-wrap items-center gap-2 mt-8 pt-6 border-t border-blue-100">
                  <Tag className="w-4 h-4 text-blue-600" />
                  {post.tags.map((tag) => (
                    <Badge key={tag} variant="outline" className="border-blue-200 text-blue-700">{tag}</Badge>
                  ))}
                </div>
              )}
            </div>
            ...
            <div className="space-y-5">
              <Card className="border-blue-100/60 shadow-md">
                <CardContent className="p-6 space-y-4">
                  <h3 className="font-bold text-blue-950">محتاج تنقل اثاثك؟</h3>
              ...
              {relatedPosts.length > 0 && (
                <Card className="border-blue-100/60">
                  <CardContent className="p-6">
                    <h3 className="font-bold text-blue-950 mb-4">مقالات ذات صلة</h3>
                    <div className="space-y-3">
                      {relatedPosts.map((rp) => (
                        <Link key={rp.id} href={`/blog/${rp.slug}`} className="flex items-start gap-3 group">
                          <div className="relative w-16 h-16 rounded-lg overflow-hidden shrink-0 border border-blue-100/60">
                            <Image src={rp.image} alt={rp.title} fill className="object-cover" sizes="64px" />
                          </div>
                          <div className="min-w-0">
                            <h4 className="text-sm font-bold text-blue-950 line-clamp-2 group-hover:text-blue-700 transition-colors">{rp.title}</h4>
